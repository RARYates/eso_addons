...
function MasterMerchant.addedSearchToItem(itemLink)
  --local adder = GetItemLinkName(itemLink)
  local adder = ''
  local requiredVeteranRank = 0
  local vrAdder = ''

  local requiredLevel = GetItemLinkRequiredLevel(itemLink)
  if GetAPIVersion() >= 100015 then
    requiredVeteranRank = GetItemLinkRequiredChampionPoints(itemLink) 
    vrAdder = GetString(MM_CP_RANK_SEARCH)
  else
    requiredVeteranRank = GetItemLinkRequiredVeteranRank(itemLink)
    vrAdder = GetString(MM_VET_RANK_SEARCH)
  end
...
function MasterMerchant:indexEverything(pause, lastItem)
  local playerName = string.lower(GetDisplayName())
  local count = 0

  local itemID, datas = next(self.salesData, lastItem)
  while itemID do
  --for itemID, datas in pairs(self.salesData) do
    ...
    for itemData, soldItems in pairs(datas) do
      for itemIndex, soldItem in ipairs(soldItems['sales']) do
        count = count + 1
        local searchBuyer = 'b' .. string.lower(tostring(soldItem['buyer']))
        local searchSeller = 's' .. string.lower(tostring(soldItem['seller']))
        local searchGuild = string.lower(tostring(soldItem['guild']))
        local guildByWords = string.gmatch(searchGuild, '%S+')
        if soldItems.itemAdderText == nil then
          soldItems.itemAdderText = self.addedSearchToItem(soldItem['itemLink'])
        end
        if soldItems.itemDesc == nil then
          soldItems.itemDesc = GetItemLinkName(soldItem['itemLink'])
        end

        local searchName = MasterMerchant.concat(string.lower(tostring(soldItems.itemDesc)), soldItems.itemAdderText)
        local nameByWords = string.gmatch(searchName, '%S+')
        local isSelfSale = playerName == string.lower(tostring(soldItem['seller']))
        ...
        --Index seller
        if self.SRIndex[searchSeller] == nil then self.SRIndex[searchSeller] = {{numberID, itemData, itemIndex}}
        else table.insert(self.SRIndex[searchSeller], {numberID, itemData, itemIndex}) end
        if isSelfSale then
          if self.SSIndex[searchSeller] == nil then self.SSIndex[searchSeller] = {{numberID, itemData, itemIndex}}
          else table.insert(self.SSIndex[searchSeller], {numberID, itemData, itemIndex}) end
        end
    ...
    if pause and count > 100 then
      local LEQ = LibStub('LibExecutionQueue-1.0')
      LEQ:ContinueWith(function () self:indexEverything(pause, itemID) end, nil)
      return
    else
      itemID, datas = next(self.salesData, itemID)
    end
  end
end

function MasterMerchant:indexHistoryTables(pause)
  self.SSIndex = {}
  self.SRIndex = {}

  self:indexEverything(pause)
end
...
function MasterMerchant:addToHistoryTables(theEvent, checkForDups)
  ...
  --[[DEBUG
  if (not checkForDups and self.salesData[theIID] and self.salesData[theIID][itemIndex]) then
    for k, v in pairs(self.salesData[theIID][itemIndex]['sales']) do
      if v.buyer == newSalesItem.buyer and
          v.guild == newSalesItem.guild and
          v.quant == newSalesItem.quant and
          v.price == newSalesItem.price and
          v.seller == newSalesItem.seller and
          v.itemLink == newSalesItem.itemLink and
          v.wasKiosk == newSalesItem.wasKiosk and
          (math.abs(v.timestamp - newSalesItem.timestamp) <= 2) then 
        d('why wernet we checking')
        d(v)
        d(newSalesItem)
        return false
      end
  ...
  if self.salesData[theIID][itemIndex] then
    self.salesData[theIID][itemIndex].itemIcon = iconForItem
  ...
  -- Index seller
  if self.SRIndex[searchSeller] == nil then self.SRIndex[searchSeller] = {{theIID, itemIndex, insertedIndex}}
  else table.insert(self.SRIndex[searchSeller], {theIID, itemIndex, insertedIndex}) end
  if isSelfSale then
    if self.SSIndex[searchSeller] == nil then self.SSIndex[searchSeller] = {{theIID, itemIndex, insertedIndex}}
    else table.insert(self.SSIndex[searchSeller], {theIID, itemIndex, insertedIndex}) end
  end
