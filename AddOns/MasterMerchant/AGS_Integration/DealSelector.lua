  ...
  local function GetFormattedPrice(price)
	...
	function ProfitFilter:GetTooltipText(state)
	  ...
	  if(minProfit and maxProfit) then
		  profitText = gettext("<<1>> - <<2>>", GetFormattedPrice(minProfit), GetFormattedPrice(maxProfit))
	  elseif(minProfit) then
		  profitText = gettext("over <<1>>", GetFormattedPrice(minProfit))
	  elseif(maxProfit) then
		  profitText = gettext("under <<1>>", GetFormattedPrice(maxProfit))
	  end
