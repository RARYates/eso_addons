...
-- Called on events EVENT_QUEST_ADVANCED & EVENT_QUEST_CONDITION_COUNTER_CHANGED
...
-- if the user toggles the "Show quest tracker" setting, update it in settings
local function OnInterfaceSettingChanged(eventCode, system, settingId) 
	if not system == SETTING_TYPE_UI then return end
	if not settingId == UI_SETTING_SHOW_QUEST_TRACKER then return end
...
function QuestTracker:Initialize()
    ...
    QUEST_TRACKER:RegisterCallback("QuestTrackerAssistStateChanged", function(unassistedData, assistedData) OnAssistChanged(self, unassistedData, assistedData) end)
...
function QuestTracker:UpdateQuest(journalIndex)
	...
	assert(questNode, "UpdateQuest failed. Quest does not exist in the quest tree")
