...
function DisplayFrame:Create(id, point, x, y, alpha, scale)
	...
	local settingID = (id < GROUP_START_FRAME) and id or Srendarr:GetGroupRaidTab() -- use the same settings for all group or raid frames (Phinix)
...
function DisplayFrame:AddAuraToDisplay(flagBurst, auraGroup, auraType, auraName, unitTag, start, finish, icon, effectType, abilityType, abilityID, stacks)
	...
	if stackingAuras[abilityID] then
		if auraLookup[unitTag][stackingAuras[abilityID].sID] ~= nil then
			if unitTag == 'player' then
				auraLookup[unitTag][stackingAuras[abilityID].sID]:Update(start, finish, stacks, true)
				return
			end
		end
	end
	...
	aura:Initialize(auraGroup, setType, auraName, unitTag, start, finish, icon, effectType, abilityType, abilityID)
...
function DisplayFrame:ConfigureAssignedAuras()
	local settingID = (self.displayID < GROUP_START_FRAME) and self.displayID or Srendarr:GetGroupRaidTab()
...
function DisplayFrame:ConfigureDragOverlay()
	...
	if self.displayID > 10 then return end -- no overlay for group frames
