	...
	for trait, effect in pairs(self.traits) do
		...
		if double and passiveTraits[data.oppositeTraits[trait]] ~= nil then
			...
			if effect == traitEffect.Bad then
				if addsBad then return matched, false, count end
				addsBad = true
				addsOpposite = true
			end
		end
...
local function ApplyLanguageSpecific()
	...
	if PotMaker.language.name == "de" then
		...
		function PotMaker.Potion:GetMasterSearchName()
			if self.searchMasterName then return self.searchMasterName end
	...
	elseif PotMaker.language.name == "zh" then
	...
	elseif PotMaker.language.name == "es" then
		function PotMaker.Potion:GetSearchName()
			if self.searchName ~= "" then return self.searchName end
			...
			if #PotMaker.masterquests > 0 and self.numTraits >= 3 and self.solvent and self.solvent.level >= 50 then
				if find(PotMaker.masterquests, self:GetSearchName()) then
					for traitName in pairs(self.traits) do
						if not zo_plainstrfind(PotMaker.masterquests, traitName) then return false end
					end
					return true
				end
			end
			if #PotMaker.quests > 0 then
				if find(PotMaker.quests, self:GetSearchName()) then return true end
			end
			return false
		end
	elseif PotMaker.language.name == "zh" then
		...
		if PotMaker.language.name == "de" then
			function PotMaker.Potion:MatchesQuest()
				if #PotMaker.masterquests > 0 and self.numTraits >= 3 and self.solvent and self.solvent.level >= 50 then
					if find(PotMaker.masterquests, self:GetMasterSearchName()) then
						if find(PotMaker.masterquests, string.format("%%A%s%%A", self:GetMasterSearchName())) then
							...
							for traitName in pairs(self.traits) do
								if not find(PotMaker.masterquests, string.format("%%A%s%%A", traitName)) then return false end
							end
							return true
						end
					end
				...
				if #PotMaker.quests > 0 then
					if PotMaker.solventMode == ITEMTYPE_POTION_BASE and find(PotMaker.quests, self:GetSearchName()) then
						if find(PotMaker.quests, string.format("%%A%s%%A", self:GetSearchName())) then return true end
					end
					if PotMaker.solventMode == ITEMTYPE_POISON_BASE and find(PotMaker.quests, self:GetMasterSearchName()) then
						if find(PotMaker.quests, string.format("%%A%s%%A", self:GetMasterSearchName())) then return true end
					end
				end
				return false
			end
			...
			function PotMaker.Potion:MatchesQuest()
				...
				if #PotMaker.quests > 0 then
					if find(PotMaker.quests, self:GetSearchName()) then
						if find(PotMaker.quests, string.format("%%A%s%%A", self:GetSearchName())) then return true end
					end
				end
				return false
...
do
	...
	function PotMaker.Potion:conformsToSearch(searchTerms)
		...
		if self.solvent ~= nil and(self.itemLink == "" and not(playerSettings.useUnknown or playerSettings.useMissing)) then return false end
		...
		if searchTermsExist or useUnknown then
			for traitName, effect in pairs(self.traits) do
				if effect == traitEffect.Bad or effect == traitEffect.VeryBad then
				...
				elseif effect == traitEffect.Good or effect == traitEffect.VeryGood then
					if useUnknown then
						for j = 1, #self.ingredients do
							if self.ingredients[j].traits[traitName] == traitEffect.None then
								-- nil ~= false
...
do
	...
	function PotMaker.Potion:createFavoriteIdentifier()
		...
		if not item1 then item1, item2, item3 = "", "", "" end
	...
	local function AddReagent(itemType, slotIndex)
		...
		if not IsAlchemySolvent(itemType) then
			...
			if playerSettings.useUnknown then
				...
				if newTraits then
					for trait in pairs(newTraits.traits) do
						if not traits[trait] then traits[trait] = traitEffect.None end
					end
				end
			end
...
do
	...
	local function LastJob()
		task:Call(SortPotions):Then(ShowPage)
	end
	...
	local function createCombinationIngredient(ingredient1, ingredient2, matches)
		...
		for trait, effect in pairs(matches) do combinationTraits[trait] = effect end
	...
	function PotMaker.findDoablePotions(searchTerms, searchSolvent, searchReagent)
		...
		local function FindDoubleSlot(ingredient1, ingredient2, amount1)
			...
			if (next(checkMatches) or useUnknown) and reagentFilter(ingredients) then
				-- for each selected solvent we make a potion
				for s = 1, #solvents do
					local currentSolvent = solvents[s]
			...
			if useThirdSlot then
				...
				task:Call( function() FindTripleSlot(combinationIngredient, ingredient1, ingredient2, amount1, amount2, allReagents[ingredient1.name].matching) end)
				task:Then( function() FindTripleSlot(combinationIngredient, ingredient1, ingredient2, amount1, amount2, allReagents[ingredient2.name].matching) end)
			end
		end
		task:Call(StartJobs)
		for _, ingredient1 in pairs(reagents) do
			...
			for k = 1, #matching1 do
				...
				if ingredient2 ~= nil and allReagents[ingredient1.name].itemId < allReagents[ingredient2.name].itemId then
					-- long life the Lua enclosure
					task:Then( function() FindDoubleSlot(ingredient1, ingredient2, amount1) end)
				end
			end
		end
		task:Then(LastJob)
	...
	function PotMaker.interactWithAlchemyStation(eventCode, craftSkill)
		if craftSkill == CRAFTING_TYPE_ALCHEMY then
			...
			if not PotMaker.atAlchemyStation or accountSettings.showAsDefault then
				if PotMaker.atAlchemyStation then LAS:SelectTab(playerSettings.lastUsedTab or PotMaker.descriptorPotion) end
			end
...
local function InternalStartSearch(quests, masterquests)
	PotMaker.quests = #quests > 8 and quests or ""
	PotMaker.masterquests = #masterquests > 8 and masterquests or ""
	...
	for questIndex = 1, MAX_JOURNAL_QUESTS do
		if IsValidQuestIndex(questIndex) then
			...
			if questType == QUEST_TYPE_CRAFTING then
				...
				quests[#quests + 1] = PotMaker:ParseQuest(activeStepText)
...
function PotMaker.RenderPage()
	...
	async:For(startIndex, endIndex):Do( function(i)
...
do
	...
	function PotMaker.PotionClicked(sender, button)
		...
		elseif button == MOUSE_BUTTON_INDEX_RIGHT and IsChatSystemAvailableForCurrentPlatform() and sender.potion and sender.potion.itemLink ~= "" then
			ClearMenu()
			if PotMaker.atAlchemyStation then
				AddCustomMenuItem(GetString(SI_HOUSING_EDITOR_SELECT), function() PotMaker.Potion.show(sender) end)
			end
			...
			if potion then
				...
				if PotionMakerSavedFavorites[potion.itemId] then
					AddCustomMenuItem(PotMaker.language.unmark_favorite, function()
						PotionMakerSavedFavorites[potion.itemId] = nil
						PotMaker.initFavorites()
						PotMaker.RenderPage()
					end )
				else
					AddCustomMenuItem(PotMaker.language.mark_favorite, function()
...
function PotMaker.initMainMenu()
	...
	POTIONMAKER_FRAGMENT:RegisterCallback("StateChange", function(oldState, newState)
		if newState == SCENE_FRAGMENT_SHOWING then
			...
			if mode ~= PotMaker.descriptorPotion and mode ~= PotMaker.descriptorPoison then mode = PotMaker.descriptorPotion end
	...
	local categoryLayoutInfo =
	{
		...
		visible = function(buttonData) return accountSettings.showMainMenuItem end,
...
function PotMaker.initSettingsMenu()
	...
	local optionsTable = {
		...
		},
		{
			...
			getFunc = function() return playerSettings.useMissing end,
			setFunc = function(value) playerSettings.useMissing = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return playerSettings.useUnknown end,
			setFunc = function(value)
				playerSettings.useUnknown = value
				if value then playerSettings.training = false end
			end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return playerSettings.training and playerSettings.useUnknown end,
			setFunc = function(value) playerSettings.training = value end,
			width = "full",

			default = PotMaker.dataDefaults.training,
			disabled = function() return not playerSettings.useUnknown end,
		},
		{
			...
			getFunc = function() return playerSettings.fakeThirdSlot end,
			setFunc = function(value) playerSettings.fakeThirdSlot = value end,
			width = "full",
...

		{
			...
			getFunc = function() return accountSettings.XPMode end,
			setFunc = function(value) accountSettings.XPMode = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return accountSettings.reagentStackOrder end,
			setFunc = function(value) accountSettings.reagentStackOrder = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return accountSettings.showMainMenuItem end,
			setFunc = function(value)
		...
		},
		{
			...
			getFunc = function() return accountSettings.showAsDefault end,
			setFunc = function(value) accountSettings.showAsDefault = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return accountSettings.suppressNewTraitDialog end,
			setFunc = function(value) accountSettings.suppressNewTraitDialog = value end,
			width = "full",
		...
		-- 	},
		{
			...
			getFunc = function() return(FCOIsMarked ~= nil or FCOIS or ItemSaver_IsItemSaved ~= nil) and accountSettings.useItemSaver end,
			setFunc = function(value) accountSettings.useItemSaver = value end,
			width = "full",
			default = PotMaker.accountDefaults.useItemSaver,
			disabled = function() return FCOIsMarked == nil and ItemSaver_IsItemSaved == nil and FCOIS == nil end,
		},
		...
		},
		{
			...
			getFunc = function() return accountSettings.filterFavoriteByTraits end,
			setFunc = function(value) accountSettings.filterFavoriteByTraits = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return accountSettings.filterFavoriteByReagents end,
			setFunc = function(value) accountSettings.filterFavoriteByReagents = value end,
			width = "full",
		...
		},
		{
			...
			getFunc = function() return accountSettings.filterFavoriteBySolvents end,
			setFunc = function(value) accountSettings.filterFavoriteBySolvents = value end,
			width = "full",
...
function PotMaker:SelectPotionOfWrit()
	...
	local writQuestDescription, masterWritQuestDescription = GetQuests()

	if self.resultListShown then self.searchAgain() end
	local hasMasterWrit = #masterWritQuestDescription > 1
	if not hasMasterWrit then
		ZO_MenuBar_SelectDescriptor(self.modeBar, PotMaker.descriptorPotion)
	end
	self.potion2ReagentFilter = not hasMasterWrit

	InternalStartSearch(writQuestDescription, masterWritQuestDescription)
	return true
end
