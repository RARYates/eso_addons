...
local pinTypeId2Tooltip = {
	...
	["ru"] = {
		...
		[Harvest.MUSHROOM] = "Грибы",
		[Harvest.FLOWER] = "Цветы",
		[Harvest.WATERPLANT] = "Водные растения",
...
function Harvest.GetLocalizedItemNames(pinTypeId, craftingRank, playerRank)
	if craftingRank then
		craftingRank = zo_max(craftingRank, 1)
	end
	local tooltip = {}
	local group
...
function Harvest.GetLocalizedTooltip( pin, mapCache )
	...
	local tradeSkill = pinTypeId2TradeSkill[pinTypeId]
	local rank
	if tradeSkill then
	...
	if level == 50 then
		level = GetUnitEffectiveLevel("player")
	end
	local tooltip = Harvest.GetLocalizedItemNames(pinTypeId, rank, level )
	local text = {}
	local counter = 0
