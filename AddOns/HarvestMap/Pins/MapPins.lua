...
function MapPins:Initialize()
	...
	-- load pin layouts
	self.layouts = Harvest.GetMapPinLayouts()
	...
	self.defaultLayout = {
		...
		tint = function(pin)
			...
			if pinTypeId then --stuck pin, this happens sometimes when FyrMM is used. see above.
				--if self.mapCache.version[pin.m_PinTag] / 10000000 >= 3 then
				--	return self.layouts[Harvest.JUSTICE].tint
				--end
	...
	-- the "real" pin types for each resource and the farming helper tour.
	-- in case of FyrMM or votan's minimap, these resource pins aren't used,
	...
	-- debug stuff for iphitos
	if Harvest.IsTrustedUser() then
		local displayName = "Merke gelöschte Nodes"
		pve = AddCheckbox(WORLD_MAP_FILTERS.pvePanel, displayName)
		pvp = AddCheckbox(WORLD_MAP_FILTERS.pvpPanel, displayName)
		imperialPvP = AddCheckbox(WORLD_MAP_FILTERS.imperialPvPPanel, displayName)
		local fun = function(button, state)
			Harvest.SetDeletionTracking(state)
		end
		ZO_CheckButton_SetToggleFunction(pve, fun)
		ZO_CheckButton_SetToggleFunction(pvp, fun)
		ZO_CheckButton_SetToggleFunction(imperialPvP, fun)
		...
		local mapFilterType = GetMapFilterType()
		if mapFilterType == MAP_FILTER_TYPE_STANDARD then
			ZO_CheckButton_SetCheckState(pve, Harvest.AreDeletionsTracked())
		elseif mapFilterType == MAP_FILTER_TYPE_AVA_CYRODIIL then
			ZO_CheckButton_SetCheckState(pvp, Harvest.AreDeletionsTracked())
		elseif mapFilterType == MAP_FILTER_TYPE_AVA_IMPERIAL then
			ZO_CheckButton_SetCheckState(imperialPvP, Harvest.AreDeletionsTracked())
		end
		
		local displayName = "Verwende 2. Löschliste"
		pve = AddCheckbox(WORLD_MAP_FILTERS.pvePanel, displayName)
		pvp = AddCheckbox(WORLD_MAP_FILTERS.pvpPanel, displayName)
		imperialPvP = AddCheckbox(WORLD_MAP_FILTERS.imperialPvPPanel, displayName)
		local fun = function(button, state)
			Harvest.SetDeleteList(state)
		end
		ZO_CheckButton_SetToggleFunction(pve, fun)
		ZO_CheckButton_SetToggleFunction(pvp, fun)
		ZO_CheckButton_SetToggleFunction(imperialPvP, fun)
		...
		local mapFilterType = GetMapFilterType()
		if mapFilterType == MAP_FILTER_TYPE_STANDARD then
			ZO_CheckButton_SetCheckState(pve, Harvest.IsDeleteListEnabled())
		elseif mapFilterType == MAP_FILTER_TYPE_AVA_CYRODIIL then
			ZO_CheckButton_SetCheckState(pvp, Harvest.IsDeleteListEnabled())
		elseif mapFilterType == MAP_FILTER_TYPE_AVA_IMPERIAL then
			ZO_CheckButton_SetCheckState(imperialPvP, Harvest.IsDeleteListEnabled())
		end
	...
	if pinTypeId == Harvest.TOUR then
	 ...
	 --[[
		...
		local pve, pvp, imperial = LMP:AddPinFilter(
			...
			false,
			Harvest.settings.savedVars.settings.isPinTypeVisible,
...
function MapPins:RefreshPins( pinTypeId )
	...
	for _, pinType in ipairs(Harvest.PINTYPES) do
...
MapPins.clickHandler = {-- debugHandler = {
	...
	},
	{
		...
		callback = function(pin)
			...
			if Harvest.settings.savedVars.settings.trusted and Harvest.AreDeletionsTracked() then
				Harvest.settings.savedVars.global.deletedNodes = Harvest.settings.savedVars.global.deletedNodes or {}
				local nodes = Harvest.settings.savedVars.global.deletedNodes
				if Harvest.IsDeleteListEnabled() then
					Harvest.settings.savedVars.global.deletedNodes2 = Harvest.settings.savedVars.global.deletedNodes2 or {}
					nodes = Harvest.settings.savedVars.global.deletedNodes2
				end
...
function MapPins:OnSettingsChanged(event, setting, ...)
	...
	elseif setting == "pinTypeSize" then
		...
		self:RefreshPins()
	elseif setting == "pinTypeTexture" then
...
Harvest.tooltipCreator = {
	creator = function( pin ) -- this function is called when the mouse is over a pin and a tooltip has to be displayed
		if not Harvest.AreDebugMessagesEnabled() then
			...
			InformationTooltip:AddLine( "Debug: " .. tostring(nodeIndex) .. ", " .. map)
			
			local saveFile = Harvest.Data:GetSaveFile( map )
			local serializedNode = saveFile.data[ map ][ pinTypeId ][ nodeIndex ]
			
			InformationTooltip:AddLine( tostring(serializedNode) )
		end
	end,
	tooltip = 1
}
