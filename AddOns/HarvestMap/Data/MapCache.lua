		...
		for j = startJ, endJ do
			...
			if division then
				for _, nodeId in pairs(division) do
					...
					if distance < globalBasedMinDistance then
		...
		for i = zo_max(0,divisionX - range), zo_min(divisionX + range, self.numDivisions-1)  do
			...
			for j = startJ, endJ do
				...
				if dx * dx + dy * dy < maxDistance then
					...
					if division then
						for _, nodeId in pairs(division) do
							if self.isCityPin[nodeId] == nil then
								self.isCityPin[nodeId] = WouldProcessMapClick(self.localX[nodeId], self.localY[nodeId])
							end
							if not self.isCityPin[nodeId] then
