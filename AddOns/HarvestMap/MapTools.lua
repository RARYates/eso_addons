...
function Harvest.Combine3DInfoWithMeasurement( mapType, map, measurement, zoneIndex)
	...
	if distanceCorrection then
		if computed then
			Harvest.settings.savedVars.global.measurements[map] = {distanceCorrection, Harvest.GetCurrentTimestamp(), zoneId, measurement.offsetX, measurement.offsetY, measurement.scaleX, measurement.scaleY}
		end
...
function Harvest.GetMap()
	...
	if lastMapTexture ~= textureName then
		...
		if textureName == "eyevea_base" then
			...
			local heistMap = Harvest.GetBaseHeistMap( textureName )
			if heistMap then
				lastMap = heistMap .. "_base"
				return lastMap
			end
		end
...
function Harvest.GetBaseHeistMap(map)
	local prefix
